...
export default class ShapeLayer {
  ...
  addRectangle(objectJson) {
    ...
    const polygonJson = {
      ...
      polygon: [{x: 0, y: 0}, widthVector, widthVector.add(heightVector), heightVector],
  ...
  addPolygon(objectJson) {
    ...
    for (const convexPolygon of convexPolygons) {
      convexPolygon.name = objectJson.name;
  ...
  addConvexPolygon(convexPolygon, properties) {
    ...
    const shape = {
      ...
      collideWidth: function(sprite) {
        ...
        const spriteOffset = spritePolygon.center.sub(this.polygon.center);

...
export default class QuadTree {
  constructor( shapes, maxLevel, maxShapes ) {
    ...
    if ( shapes.length < maxShapes || maxLevel <= 1 ) {
      ...
      const shapesAABB = AABB.fromAABBs(
          shapes.map(shape => shape.polygon.aabb));
      const pivot = shapesAABB.center();
      this.pivot = pivot;
      let straddling = [], topLeft = [], topRight = [], bottomLeft = [],
          bottomRight = [];
      ...
      if ( bottomRight.length > 0 ) {
        this.bottomRightNode = new QuadTree(bottomRight, maxLevel - 1,
            maxShapes);
      }
    }
  }
